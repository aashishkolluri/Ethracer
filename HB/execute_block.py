...
def function_accept( op, stack, trace, debug):
	op_name = op['o']
	op_val = op['input']

	return True, False

def function_throw( op, stack, trace, debug ):
...
def function_sstore( op, stack, trace, debug):
...
def new_state(stack, storage, sha3_dict, sha3_values, mmemory, trace, data):
	datastructures = {}
...
def  run_one_check( max_call_depth, max_jump_depth, search_enhance, ops, contract_address, function1, function2, key, noHB, debug, read_from_blockchain , datastructures = {}):
	...
	if 1 == key:
		...
		if search_enhance:
			MyGlobals.s.push()
			execute_one_block(ops,stack,0, trace, storage, mmemory, data, configurations, sha3_dict, sha3_values, ['SSTORE', 'CALL', 'DELEGATECALL', 'CALLCODE', 'SUICIDE'], function_sstore, 0, 0, function_hash, False, function1, function2, contract_address,key, search_enhance, noHB, debug, read_from_blockchain )
			MyGlobals.s.pop()
		...
		if noHB:
			MyGlobals.s.push()
			execute_one_block(ops,stack,0, trace, storage, mmemory, data, configurations, sha3_dict, sha3_values, ['STOP', 'RETURN', 'SUICIDE'], function_accept, 0, 0, function_hash, True, function1, function2, contract_address,key, search_enhance, noHB, debug, read_from_blockchain )
			MyGlobals.s.pop()	
			return		
	
		MyGlobals.s = MyGlobals.s1	
		MyGlobals.s.push()
		execute_one_block(ops,stack,0, trace, storage, mmemory, data, configurations, sha3_dict, sha3_values, ['STOP', 'RETURN'], function_accept, 0, 0, function_hash, False, function1, function2, contract_address,key, search_enhance, noHB, debug, read_from_blockchain )
		MyGlobals.s.pop()
	...
	if 2 == key:
		...
		execute_one_block(ops,stack,0, trace, storage2, mmemory, data2, configurations, sha3_dict2, sha3_values2,  ['STOP', 'RETURN'], function_accept, 0, 0, function_hash, True, function1, function2, contract_address,key, search_enhance, noHB, debug, read_from_blockchain )
		MyGlobals.s.pop()
	...
	if 3 == key:
		...
		MyGlobals.s.push()
		execute_one_block(ops,stack,0, trace, storage3, mmemory, data3, configurations, sha3_dict3, sha3_values3,  ['JUMP', 'JUMPI', 'REVERT'], function_throw, 0, 0, function_hash, True, function1, function2, contract_address, key, search_enhance, noHB, debug, read_from_blockchain )
		MyGlobals.s.pop()
		...
		if not MyGlobals.solution_found:
			...
			MyGlobals.s.push()
			execute_one_block(ops,stack,0, trace, storage3, mmemory, data3, configurations, sha3_dict3, sha3_values3,  ['STOP', 'RETURN'], function_accept, 0, 0, function_hash, False, function1, function2, contract_address, key, search_enhance, noHB, debug, read_from_blockchain )
			MyGlobals.s.pop()
	...
	if 4 == key:
		...
		MyGlobals.s.push()
		MyGlobals.s.push()
		execute_one_block(ops,stack,0, trace, storage4, mmemory, data4, configurations, sha3_dict4, sha3_values4,  ['JUMP', 'JUMPI', 'REVERT'], function_throw, 0, 0, function_hash, True, function1, function2, contract_address, key, search_enhance, noHB, debug, read_from_blockchain )
		MyGlobals.s.pop()
...
def execute_one_block( ops , stack , pos , trace, storage, mmemory, data, configurations,  sha3_dict,  sha3_values, search_op, search_function, jumpdepth, calldepth, function_hash, find_solution, function1, function2, contract_address,  key, search_enhance, noHB, debug, read_from_blockchain):
	...
	while (first or newpos != pos) and not MyGlobals.stop_search:
		...
		if ops[pos]['o'] in search_op:
			...
			if MyGlobals.search_condition_found and not search_enhance:
				add_additional_conditions(MyGlobals.s, sha3_values)
			...
			if MyGlobals.search_condition_found and (not search_enhance) and (not noHB) and ((key ==1 or key ==2) or (key ==3 and not find_solution)) and (not jump_condition):
				...
				datastructures = new_state(stack, storage, sha3_dict, sha3_values, mmemory, trace, data)
				MyGlobals.s.push()
				run_one_check( 1, MyGlobals.max_jumpdepth_in_normal_search, search_enhance, ops, contract_address, function1, function2,  key+1, noHB, debug, read_from_blockchain , datastructures)
 				MyGlobals.s.pop()
				return
			...
			if si['o'] == 'JUMPI':
				...
				try:
					...
					if (MyGlobals.jumpi_switch and search_enhance):
						...
						execute_one_block(ops,stack2,	pos + 1, 	trace2, storage2, 	mmemory2, data2, configurations,  sha3_dict2, sha3_values2, search_op, search_function, jumpdepth+1, calldepth,  function_hash, find_solution,function1, function2, contract_address,  key, search_enhance, noHB, debug, read_from_blockchain )
						MyGlobals.search_condition_found = False
				...
				try:
					...
					if (MyGlobals.jumpi_switch and search_enhance ):
						...
						execute_one_block(ops,stack2,	new_position, 	trace2, storage2, 	mmemory2, data2, configurations,  sha3_dict2, sha3_values2, search_op, search_function,  jumpdepth, calldepth, function_hash, find_solution,function1, function2, contract_address, key,  search_enhance, noHB, debug, read_from_blockchain )
						MyGlobals.search_condition_found = False
			...
			# It can be CALLDATALOAD
				...
				if match:
					...
					for one_branch_size in branch_array_size:
						...
						execute_one_block(ops,stack2,	pos+1, 	trace2, storage2, 	mmemory2, data2, configurations, sha3_dict2, sha3_values2,	search_op, search_function,  jumpdepth, calldepth, function_hash, find_solution,function1, function2, contract_address, key, search_enhance,  noHB, debug, read_from_blockchain )
						MyGlobals.search_condition_found = False
						MyGlobals.s.pop()
